...
public class ContactServiceImpl implements ContactService {
    ...
    public Set<User> getContacts(User user) {
        if (!ContactServiceUtils.validateUser(user)) {
        ...
        return cache.getContactsStorage().getRelationsList().stream()
                .filter(ContactServiceUtils.getRelationsForUser(userName))
                .map(ContactServiceUtils.getFriendForUser(userName))
                .map(cache::getUser)
    ...
    public Set<RelationRequest> getInboundRequests(User user) {
        ...
        return cache.getContactsStorage().getRequestsList().stream()
                .filter(ContactServiceUtils.getInboundRequestsForUser(userName))
                .collect(Collectors.toSet());
    }
    ...
    public Set<RelationRequest> getOutboundRequests(User user) {
        ...
        String userName = user.getName();
        return cache.getContactsStorage().getRequestsList().stream()
                .filter(ContactServiceUtils.getOutboundRequestsForUser(userName))
                .collect(Collectors.toSet());
    }
